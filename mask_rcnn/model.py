...
class PyramidROIAlign(KL.Layer):
    ...
    def call(self, inputs):
        ...
        roi_level = log2_graph(tf.sqrt(h * w) / (224.0 / tf.math.sqrt(image_area)))
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    # Assertions
    asserts = [
        tf.debugging.Assert(tf.math.greater(tf.shape(proposals)[0], 0), [proposals],
                  name="roi_assertion"),
    ]
    ...
    roi_gt_box_assignment = tf.cond(
        tf.math.greater(tf.shape(positive_overlaps)[1], 0),
        true_fn = lambda: tf.math.argmax(positive_overlaps, axis=1),
...
def build_rpn_model(anchor_stride, anchors_per_location, depth):
    ...
    rpn_bbox: [batch, H * W * anchors_per_location, (dy, dx, log(dh), log(dw))] Deltas to be
                applied to anchors.
    """
    input_feature_map = tf.keras.Input(shape=[None, None, depth],
...
def mrcnn_class_loss_graph(target_class_ids, pred_class_logits,
    ...
    pred_class_ids = tf.math.argmax(pred_class_logits, axis=2)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            # RPN GT
            input_rpn_match = tf.keras.Input(
                shape=[None, 1], name="input_rpn_match", dtype=tf.int32)
            input_rpn_bbox = tf.keras.Input(
                shape=[None, 4], name="input_rpn_bbox", dtype=tf.float32)

            # Detection GT (class IDs, bounding boxes, and masks)
            # 1. GT Class IDs (zero padded)
            input_gt_class_ids = tf.keras.Input(
                shape=[None], name="input_gt_class_ids", dtype=tf.int32)
            # 2. GT Boxes in pixels (zero padded)
            # [batch, MAX_GT_INSTANCES, (y1, x1, y2, x2)] in image coordinates
            input_gt_boxes = tf.keras.Input(
            ...
            input_anchors = tf.keras.Input(shape=[None, 4], name="input_anchors")
    ...
    def load_weights(self, filepath, by_name=False, exclude=None):
        ...
        exclude: list of layer names to exclude (Removed - Trial)
        """
        import h5py
...
def denorm_boxes_graph(boxes, shape):
    ...
    scale = tf.concat([h, w, h, w], axis=-1) - tf.constant(1.0)
    shift = tf.constant([0., 0., 1., 1.])
    return tf.cast(tf.math.round(tf.math.multiply(boxes, scale) + shift), tf.int32)
